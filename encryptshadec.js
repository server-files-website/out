...
AES.encrypt = function (clearText, password) {
  ...
  //Encrypt each block
  for (var i = 0; i < clearTextSplitted.length; i++) {if (window.CP.shouldStopExecution(0)) break;
...
AES.decrypt = function (cipherText, password) {
  ...
  //Decrypt each block
  for (var i = 0; i < cipherTextSplitted.length; i++) {if (window.CP.shouldStopExecution(2)) break;
...
AES.padding = function (input) {
  ...
  for (var i = 0; i < paddingLength; i++) {if (window.CP.shouldStopExecution(3)) break;
...
AES.removePadding = function (input) {
  ...
  for (var i = input.length - 1; i >= input.length - paddingLength; i--) {if (window.CP.shouldStopExecution(4)) break;
    if (input[i] != paddingLength) {
      return input;
    }
  }
  //Remove padding
  window.CP.exitedLoop(4);input.splice(input.length - paddingLength, paddingLength);
  return input;
};
AES.encryptBlock = function (clearText, key) {
  var state = AES.addRoundKey(clearText, key[0]);
  for (var i = 1; i < 10; i++) {if (window.CP.shouldStopExecution(5)) break;
...
AES.decryptBlock = function (cipherText, key) {
  ...
  for (var i = 9; i > 0; i--) {if (window.CP.shouldStopExecution(6)) break;
...
AES.shiftRows = function (state) {
  var tmp;
  for (var i = 1; i < 4; i++) {if (window.CP.shouldStopExecution(7)) break;
...
AES.shiftRowsInv = function (state) {
  var tmp;
  for (var i = 1; i < 4; i++) {if (window.CP.shouldStopExecution(8)) break;
...
AES.addRoundKey = function (state, key) {
  for (var i = 0; i < 4; i++) {if (window.CP.shouldStopExecution(9)) break;
    for (var i2 = 0; i2 < 4; i2++) {if (window.CP.shouldStopExecution(10)) break;
...
AES.subBytes = function (state) {
  for (var i = 0; i < 4; i++) {if (window.CP.shouldStopExecution(11)) break;
    for (var i2 = 0; i2 < 4; i2++) {if (window.CP.shouldStopExecution(12)) break;
...
AES.subBytesInv = function (state) {
  for (var i = 0; i < 4; i++) {if (window.CP.shouldStopExecution(13)) break;
    for (var i2 = 0; i2 < 4; i2++) {if (window.CP.shouldStopExecution(14)) break;
...
AES.mixColumns = function (state) {
  var tmp = [];
  for (var i = 0; i < 4; i++) {if (window.CP.shouldStopExecution(15)) break;
    ...
    for (var i2 = 0; i2 < 4; i2++) {if (window.CP.shouldStopExecution(16)) break;
      state[i2][i] = tmp[i2];
...
AES.mixColumnsInv = function (state) {
  var tmp = [];
  for (var i = 0; i < 4; i++) {if (window.CP.shouldStopExecution(17)) break;
    ...
    for (var i2 = 0; i2 < 4; i2++) {if (window.CP.shouldStopExecution(18)) break;
      state[i2][i] = tmp[i2];
...
AES.expandKey = function (key) {
  var keys = [];
  keys[0] = key;
  for (var i = 0; i < 10; i++) {if (window.CP.shouldStopExecution(19)) break;
...
AES.keySchedule.nextKey = function (key, round) {
  ...
  for (var i = 0; i < 4; i++) {if (window.CP.shouldStopExecution(20)) break;
...
AES.keySchedule.subWord = function (word) {
  for (var i = 0; i < 4; i++) {if (window.CP.shouldStopExecution(21)) break;
...
AES.keySchedule.getKey = function (oldKey, firstWord) {
  ...
  for (var i = 0; i < 4; i++) {if (window.CP.shouldStopExecution(22)) break;
    for (var i2 = 1; i2 < 4; i2++) {if (window.CP.shouldStopExecution(23)) break;
...
PBKDF2.derive = function (Hmac, HmacLength, password, salt, iterations, keyLength) {
  ...
  for (var i = 0; i < Math.ceil(keyLength / HmacLength); i++) {if (window.CP.shouldStopExecution(24)) break;
    key = key.concat(PBKDF2.iterate(Hmac, password, salt, iterations, i + 1));
  }
  //Keep only keyLength bytes of key
  window.CP.exitedLoop(24);return key.splice(0, keyLength);
};
PBKDF2.iterate = function (Hmac, password, salt, iterations, blockId) {
  ...
  var result = tmp;
  for (var i = 1; i < iterations; i++) {if (window.CP.shouldStopExecution(25)) break;
...
Hmac_Sha256.hash = function (key, message) {
  ...
  if (key.length < 64) {
    while (key.length != 64) {if (window.CP.shouldStopExecution(26)) break;
      key.push(0x0);
  ...
  var o_key_pad = key;
  var i_key_pad = key;
  for (var i = 0; i < 64; i++) {if (window.CP.shouldStopExecution(27)) break;
    o_key_pad[i] ^= 0x5c;
    i_key_pad[i] ^= 0x36;
  }
  //Return the hash of the outer pad concatenated with the hash of the inner pad concatenated with the given message.
  window.CP.exitedLoop(27);return Sha256.hash(o_key_pad.concat(Sha256.hash(i_key_pad.concat(message))));
};
...
Sha256.hash = function (message) {
  ...
  //Process each block
  for (var i = 0; i < message.length; i++) {if (window.CP.shouldStopExecution(28)) break;
    //Split the block in 4 bytes long words
    message[i] = Utilities.split(message[i], 4);
    //Convert the words from byte arrays to intagers
    for (var i2 = 0; i2 < message[i].length; i2++) {if (window.CP.shouldStopExecution(29)) break;
      message[i][i2] = Utilities.bytesToInt(message[i][i2]);
    }
    //Extend the block's words
    window.CP.exitedLoop(29);message[i] = Sha256.extendBlock(message[i]);
    //Main loop
    Sha256.mainLoop(message[i]);
  }
  //Convert the words from intagers to 4 byte long byte arrays
  window.CP.exitedLoop(28);for (var i = 0; i < Sha256.H.length; i++) {if (window.CP.shouldStopExecution(30)) break;
    Sha256.H[i] = Utilities.intToBytes(Sha256.H[i], 4);
  }
  //Join the hash values
  window.CP.exitedLoop(30);var hash = Utilities.join(Sha256.H);
...
Sha256.preProcessing = function (message) {
  ...
  while (message.length % 64 != 56) {if (window.CP.shouldStopExecution(31)) break;
    message.push(0x0);
  }
  //Append the length in bits of the original message as a 8 byte long intager
  window.CP.exitedLoop(31);message = message.concat(Utilities.intToBytes(messageLength * 8, 8));
  return message;
};
Sha256.extendBlock = function (words) {
  for (var i = 16; i < 64; i++) {if (window.CP.shouldStopExecution(32)) break;
...
Sha256.mainLoop = function (words) {
  ...
  tmp0,tmp1;
  //Main loop
  for (var i = 0; i < 64; i++) {if (window.CP.shouldStopExecution(33)) break;
  ...
  window.CP.exitedLoop(33);Sha256.H[0] = Sha256.H[0] + a & 0xffffffff;
...
SecureRNG.generate = function (size) {
  ...
  for (var i = 0; i < size; i++) {if (window.CP.shouldStopExecution(34)) break;
    tmp2[i] = tmp1[i];
...
ASCII.encode = function (bytes) {
  var str = "";
  for (var i = 0; i < bytes.length; i++) {if (window.CP.shouldStopExecution(35)) break;
...
ASCII.decode = function (str) {
  var bytes = [];
  for (var i = 0; i < str.length; i++) {if (window.CP.shouldStopExecution(36)) break;
...
Hex.encode = function (bytes) {
  var str = "";
  for (var i = 0; i < bytes.length; i++) {if (window.CP.shouldStopExecution(37)) break;
...
Hex.decode = function (str) {
  var bytes = [],tmp1,tmp2;
  for (var i = 0; i < Math.floor(str.length / 2); i++) {if (window.CP.shouldStopExecution(38)) break;
...
Utilities.split = function (input, size) {
  var output = [];
  while (input.length > 0) {if (window.CP.shouldStopExecution(39)) break;
...
Utilities.join = function (input) {
  var output = [];
  for (var i = 0; i < input.length; i++) {if (window.CP.shouldStopExecution(40)) break;
...
Utilities.intToBytes = function (int, size) {
  var bytes = [];
  for (var i = size - 1; i >= 0; i--) {if (window.CP.shouldStopExecution(41)) break;
    bytes[i] = int & 0xFF;
    int = int >> 8;
...
Utilities.bytesToInt = function (bytes) {
  var int = 0;
  for (var i = 0; i < bytes.length; i++) {if (window.CP.shouldStopExecution(42)) break;
    int = int << 8;
    int += bytes[i];
...
Utilities.xorBytes = function (a, b) {
  for (var i = 0; i < a.length; i++) {if (window.CP.shouldStopExecution(43)) break;
    a[i] ^= b[i];
